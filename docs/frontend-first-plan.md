...
- 前端 `npm run build` 已通过，生产构建可生成 `frontend/dist/`。
- 后端 `mvn test` 已通过，8 个测试覆盖认证、公开健康检查、用户隔离、任务状态、看板统计、AI fallback 和非法参数。
- 后端默认 MySQL profile 已使用本地 `root / 123456` 启动成功。
- 前端设置 `VITE_USE_MOCK=false` 后已启动真实 API 模式。
- 真实 API 已走通注册、当前用户、科目、任务创建与完成、看板统计和 AI fallback 流程。

注意：MySQL 数据目录以本机配置为准；当前机器的 MySQL 数据写入 C 盘，后续继续建库建表或新增数据前仍需确认 C 盘增量。
